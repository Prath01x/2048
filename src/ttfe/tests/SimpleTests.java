...
public class SimpleTests {
 ...
 public void movepoo3(){
	...
	@Test
    public void movePossibleDirectionNorthNo() {
        // Clear the board
        for (int row = 0; row < 4; row++) {
            for (int col = 0; col < 4; col++) {  // Fixed: setting all 4 columns to 0
                game.setPieceAt(row, col, 0);
            }
        }
        ...
        assertEquals("No space available and no tiles can merge", false, game.isMovePossible(MoveDirection.NORTH));
    }

    @Test
    public void movePossibleDirectionNorthYes() {
        // Clear the board
        for (int row = 0; row < 4; row++) {
            for (int col = 0; col < 4; col++) {
                game.setPieceAt(row, col, 0);
        ...
        // Set up a board with empty spaces, so moves are possible
        game.setPieceAt(1, 0, 4);
        game.setPieceAt(2, 0, 8);
        game.setPieceAt(3, 0, 16);
        ...
        assertEquals("Space available but no tiles can merge", true, game.isMovePossible(MoveDirection.NORTH));
    }

    @Test
    public void movePossibleDirectionNorthYesTiles() {
        // Clear the board
        for (int row = 0; row < 4; row++) {
            for (int col = 0; col < 4; col++) {
                game.setPieceAt(row, col, 0);
        ...
        // Set up a board with tiles that can merge
        game.setPieceAt(0, 0, 2);
        game.setPieceAt(1, 0, 4);
        game.setPieceAt(2, 0, 8);
        game.setPieceAt(3, 0, 16);
        ...
        assertEquals("No space available but tiles can merge", true, game.isMovePossible(MoveDirection.NORTH));
    }
}
